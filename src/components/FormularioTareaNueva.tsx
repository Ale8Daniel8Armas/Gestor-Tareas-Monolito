...
type Props = {
  type: 'crear' | 'editar';
  data?: TareaInterface;
  onEventCreated?: (event: { id: string; title: string; start: string }) => void;
}

const FormularioTareaNueva = ({ type, data, onEventCreated }: Props) => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      if (type === 'crear') {
        ...
        if (tareaNueva) {
          onEventCreated?.({
            id: tareaNueva._id, // Asegúrate de que `_id` sea el identificador único
            title: values.titulo,
            start: format(values.fechaACompletar, "yyyy-MM-dd")
          });
          router.push('/home');
          form.reset();
        }
